  ...
  try {
    ...
    switch (type) {
      case 'overview':
        ...
        const newPosts = allPosts.filter((post: { publishedAt: string | number | Date; }) => 
          new Date(post.publishedAt) >= startDate
        );
        
        // Calculate average post size (content length)
        const totalSize = allPosts.reduce((sum: any, post: { content: string | any[]; }) => sum + post.content.length, 0);
      ...
      case 'posts':
        ...
        data = allPosts.map((post: { slug: any; title: any; publishedAt: any; category: any; featured: any; createdAt: any; content: string | any[]; }) => ({
          slug: post.slug,
          title: post.title,
      ...
      case 'featured':
        ...
        // Format posts for display
        data = featuredPosts.map((post: { slug: any; title: any; publishedAt: any; category: any; featured: any; createdAt: any; content: string | any[]; }) => ({
          slug: post.slug,
          title: post.title,
          date: post.publishedAt,
          category: post.category || 'Uncategorized',
