...
export async function GET(request: NextRequest) {
  return apiAuthMiddleware(request, async (req, apiKeyId) => {
    try {
...
export async function POST(request: NextRequest) {
  return apiAuthMiddleware(request, async (req, apiKeyId) => {
    try {
      ...
      // In a real implementation, you might associate API keys with specific authors
      // or include the author ID in the request
      const defaultAuthorId = 1; // Replace with logic to get an appropriate author ID
      ...
      const post = await postsService.createPost(
        ...
        false,
      ); // Don't send Discord notifications for API-created posts
