  ...
  async validateApiKey(apiKeyString: string): Promise<ApiKeyValidationResult> {
    try {
      ...
      const [apiKey] = await db
        .select()
        .from(apiKeys)
        .where(and(eq(apiKeys.prefix, prefix), eq(apiKeys.enabled, true)));
...
class MockApiKeyService {
  ...
  async updateApiKey(id: number, updates: any): Promise<ApiKeyWithoutHash | null> {
  ...
  async getApiLogs(filters: ApiLogFilters): Promise<any[]> {
    const logs = await this.mockLogsService.find((log) => log.apiKeyId === filters.apiKeyId);

    // Apply date filters if provided
    let filteredLogs = logs;

    if (filters.startDate) {
      filteredLogs = filteredLogs.filter((log) => new Date(log.createdAt) >= filters.startDate!);
    }

    if (filters.endDate) {
      filteredLogs = filteredLogs.filter((log) => new Date(log.createdAt) <= filters.endDate!);
    }
  ...
  async getApiUsageStats(options: ApiUsageStatsOptions): Promise<ApiUsageStats> {
    const logs = await this.mockLogsService.find((log) => log.apiKeyId === options.apiKeyId);

    // Apply date filters if provided
    let filteredLogs = logs;

    if (options.startDate) {
      filteredLogs = filteredLogs.filter((log) => new Date(log.createdAt) >= options.startDate!);
    }

    if (options.endDate) {
      filteredLogs = filteredLogs.filter((log) => new Date(log.createdAt) <= options.endDate!);
    }
