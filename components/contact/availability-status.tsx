      ...
      <CardContent className="space-y-4">
        {loading && !error ? (
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <div
              className={`h-3 w-3 rounded-full mt-2 inline-block mr-2 ${new Date().getHours() >= 9 && new Date().getHours() < 19 ? 'bg-green-500' : 'bg-yellow-500'}`}
            ></div>
          ...
          <>
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div
                  className={`h-3 w-3 rounded-full mr-2 ${getStatusColor(discordStatus.status)}`}
                ></div>
                <span className="font-medium">{getStatusDisplayName(discordStatus.status)}</span>
              </div>
