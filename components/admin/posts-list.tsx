...
export default function PostsList({ userRole, userPermissions }: PostsListProps) {
  ...
  const toggleFeatured = async (post: BlogPost) => {
    try {
      ...
      setPosts(posts.map(p => 
        p.slug === post.slug ? { ...p, featured: !post.featured } : p
      ))
  ...
  const canEditPost = (post: BlogPost) => {
    ...
    if (userPermissions.canEditOwnPosts && post.author && post.author.id === userPermissions.userId) {
      return true
    }
  ...
  const canDeletePost = (post: BlogPost) => {
    ...
    if (userPermissions.canDeleteOwnPosts && post.author && post.author.id === userPermissions.userId) {
      return true
    }
